...
export type AnimeAiringStatusLabelType = keyof typeof ANIME_AIRING_STATUS_DISPLAY_VALUE_TO_PARAM_OBJECT;
export type AnimeAiringStatusParamType = typeof ANIME_AIRING_STATUS_DISPLAY_VALUE_TO_PARAM_OBJECT[AnimeAiringStatusLabelType];
...
export function getAnimeAiringStatusParamFromLabel(label: AnimeAiringStatusLabelType): AnimeAiringStatusParamType {
    return ANIME_AIRING_STATUS_DISPLAY_VALUE_TO_PARAM_OBJECT[label];
}

export function getAnimeAiringStatusLabelFromParam(param: DefinedAnimeAiringStatusParamType): AnimeAiringStatusLabelType {
    for (const [label, value] of Object.entries(ANIME_AIRING_STATUS_DISPLAY_VALUE_TO_PARAM_OBJECT)) {
